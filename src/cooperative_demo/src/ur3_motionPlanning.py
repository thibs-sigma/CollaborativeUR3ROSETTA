...
"""
    ur3_motionPlanning.py - Version 0.1 03-10-2019
...
class ur3_motionPlanning:
    def __init__(self):
        # ROS stuff
        rospy.init_node("ur3_motionPlanning", anonymous=False)
        self.cxy_sub = rospy.Subscriber('cxy', Tracker, self.tracking_callback, queue_size=1) # Position center of object in image
        self.cxy_pub = rospy.Publisher('cxy1', Tracker, queue_size=1)
        self.end_effector_pose_pub = rospy.Publisher('end_effector_pose', Pose, queue_size=1)
        ...
        rospy.loginfo("Starting node ur3_motionPlanning")
        ...
        self.arm.set_planning_time(0.5) #Increased planning time for simulation
        ...
        # UR3 values
        self.default_joint_states[0] = 0.1388552188873291
        self.default_joint_states[1] = -0.569965124130249
        self.default_joint_states[2] = 0.17023944854736328
        self.default_joint_states[3] = -1.1359660625457764
        self.default_joint_states[4] = -1.5523438453674316
        self.default_joint_states[5] = -1.4006805419921875
        ...
        self.end_joint_states[0] = -3.65 # UR5 demo value
        # self.end_joint_states[0] = 0.1388552188873291 # UR3
        # self.end_joint_states[1] = -1.3705

        # self.transition_pose = deepcopy(self.default_joint_states)
        # self.transition_pose[0] = -3.65 # UR5 value
        # self.transition_pose[0] = 0.1388552188873291
        # self.transition_pose[4] = -1.95 # UR5 value
        # self.transition_pose[4] = -1.5523438453674316
    ...
    def tracking_callback(self, msg):
        ...
        if len(self.pointx)>9:
            self.track_flag = True # More than 9 waypoints to follow, track the object
        if self.phase == 2: # Enter "grasp" phase
            self.track_flag = False # Stop tracking
            self.phase = 1 # Execute motion

        if (self.track_flag and -0.6 < self.waypoints[0].position.x and self.waypoints[0].position.x < 0.6):
            self.execute() # Execute motion
        ...
        # Publish the end-effector pose (debug purposes)
        self.end_effector_pose_pub.publish(self.arm.get_current_pose().pose)
    ...
    def execute(self):
        if self.track_flag:
            # Debug
            # rospy.loginfo("--- ENTERING LINE 210 execution ---")
            ...
            if len(self.pointx)>8:
                if len(self.pointx)==9:
                    # Debug
                    rospy.loginfo("--- 9 WAYPOINTS TO FOLLOW ---")

                    x_speed = np.mean(np.asarray(self.pointx[4:8]) - np.asarray(self.pointx[3:7]))
                    wpose.position.x += 2 * x_speed
                    wpose.position.z = 0.05 # change z pose here (grasp the object?)


                else:
                    if len(self.pointx)==11:
                        # Debug
                        rospy.loginfo("--- 11 WAYPOINTS TO FOLLOW ---")
            ...
            # Set the next waypoint to the right 0.5 meters
                ...
                wpose.position.z = 0.25 # -- Change height pickup here --
                #wpose.position.z = 0.15 # Original height
                #wpose.position.z = 0.4005 # Max height (?)

            if self.phase == 1:

                # Debug
                rospy.loginfo("--- PHASE 1: MOVING ROBOT ---")
            ...
            """moveit_commander.move_group.MoveGroupCommander.compute_cartesian_path(
                    self, waypoints, eef_step, jump_threshold, avoid_collisions= True)
...
motionPlanning=ur3_motionPlanning()
