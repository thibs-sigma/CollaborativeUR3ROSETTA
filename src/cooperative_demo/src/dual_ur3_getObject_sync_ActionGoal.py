...
def UR3_setup():
    ...
    if reset == True:
        while status_controller_UR3_1.status_list[1].status != 3 and status_controller_UR3_2.status_list[1].status != 3 :
            rospy.logwarn_throttle(5, 'Executing trajectory...')

        rospy.logwarn('Pose reset! Going to next step...')

    else:
        while status_controller_UR3_1.status_list[0].status != 3 and status_controller_UR3_2.status_list[0].status != 3 :
            rospy.logwarn_throttle(5, 'Executing trajectory...')
...
def UR3_motionPlanning():
    ...
    # Convert joint_trajectory message to action goal
    joint_trajectory_UR3_1 = make_trajectory_goal_msg(joint_trajectory_plan=plan_UR3_1, secs=1, dt=0.1, frame_id=reference_frame_UR3_1)
    rospy.loginfo('Trajectory UR3_1 generated!')

    joint_trajectory_UR3_2 = make_trajectory_goal_msg(joint_trajectory_plan=plan_UR3_2, secs=1, dt=0.1, frame_id=reference_frame_UR3_2)
    rospy.loginfo('Trajectory UR3_2 generated!')
...
def lift_after_grasping():
    ...
    while status_controller_UR3_1.status_list[2].status != 3 and status_controller_UR3_2.status_list[2].status != 3 :
        rospy.logwarn_throttle(5, 'Executing trajectory...')
